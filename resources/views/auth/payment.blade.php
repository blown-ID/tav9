    ...
    <div class="container mt-3">
        ...
        <ol>
            ...
            <li>
                ...
                mengirimkan bukti transfer <a target="_blank" @if($role==="SMP" )
                    href="https://wa.me/62{{ $settings->cp_smp }}?text=Assalamualaikum, saya ingin konfirmasi pembayaran calon siswa {{ $role }} dengan nama {{ Auth::user()->nama }} dan alamat email {{ Auth::user()->email }}. Terima kasih."
                    @elseif($role==="SMA" )
                    href="https://wa.me/62{{ $settings->cp_sma }}?text=Assalamualaikum, saya ingin konfirmasi pembayaran calon siswa {{ $role }} dengan nama {{ Auth::user()->nama }} dan alamat email {{ Auth::user()->email }}. Terima kasih."
                    @elseif($role==="SMK" )
                    href="https://wa.me/62{{ $settings->cp_smk }}?text=Assalamualaikum, saya ingin konfirmasi pembayaran calon siswa {{ $role }} dengan nama {{ Auth::user()->nama }} dan alamat email {{ Auth::user()->email }}. Terima kasih."
                    @endif class="btn text-white" style="background-color: purple;">disini</a> dan menunggu validasi
                admin
                maksimal selama 1x12 jam.
            </li>
            <li>
